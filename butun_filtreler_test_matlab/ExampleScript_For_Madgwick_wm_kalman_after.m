...
for t = 1:length(time)
    [Quaternion] = madgwick_wm(Quaternion, Gyroscope(t,:) * (pi/180), Accelerometer(t,:), Magnetometer(t,:));	% gyroscope units must be radians
    quaternion(t, :) = Quaternion;
end

euler = quatern2euler(quaternConj(quaternion)) * (180/pi);	% use conjugate for sensor frame relative to Earth and convert to degrees.
%% Process euler data through kalman algorithm
for t = 1:length(time)
eulert = euler(t,:)';
[psi theta phi]=Kalman_Filter_after(eulert);
kalman_euler(t,:)=[psi theta phi];
end
...
hold on;
plot(time, kalman_euler(:,1), 'r');
plot(time, kalman_euler(:,2), 'g');
plot(time, kalman_euler(:,3), 'b');
title('Euler angles');
xlabel('Time (s)');
ylabel('Angle (deg)');
legend('\phi', '\theta', '\psi');
