...
for t = 1:length(time)
    [Quaternion] = mahoney_wm(Quaternion, Gyroscope(t,:) * (pi/180), Accelerometer(t,:), Magnetometer(t,:));	% gyroscope units must be radians
    quaternion(t, :) = Quaternion;
end
euler = quatern2euler(quaternConj(quaternion)) * (180/pi);	% use conjugate for sensor frame relative to Earth and convert to degrees.

%% put euler data through kalman algorithm
for t = 1:length(time)
eulert = euler(t,:)';
